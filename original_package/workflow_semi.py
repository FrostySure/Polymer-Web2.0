...
class Workflow_semi():
    def __init__(self, output_file, pc_list, npoly,  linkatoms, box_size_list, order_nchain,disorder_nchain,  mix_style='n',npt_step=1000, nvt_step=5000, ncore='6'):
        self.pc_list = pc_list
        self.npoly = npoly
        self.order_nchain = order_nchain
        self.disorder_nchain = disorder_nchain
    ...
    def step_1(self):
        # 写高分子lt文件
        all_chians=0
        for nchain in self.order_nchain:
            all_chians+=int(nchain)        
        for nchain in self.disorder_nchain:
            all_chians+=int(nchain)
        all_chians=[str(all_chians)]
        demo_model = write_lts.Car2lt(pc_list=self.pc_list, npoly=self.npoly, tes_chain=self.disorder_nchain, box_size_list=self.box_size_list,
                                        lbox_size_list=self.box_size_list,linkatom=self.linkatoms, mix_style=self.mix_style, output_file=self.output_file)
        demo_model.main()

        demo_model = write_order_lts.Car2lt(pc_list=self.pc_list, npoly=self.npoly, tes_chain=self.order_nchain,box_size_list=self.box_size_list,
                                        lbox_size_list=self.box_size_list,linkatom=self.linkatoms, mix_style=self.mix_style, output_file=self.output_file)
        demo_model.main()
        demo_model = write_order_lts.Car2lt(pc_list=self.pc_list, npoly=self.npoly, tes_chain=all_chians,box_size_list=self.box_size_list,
                                        lbox_size_list=self.box_size_list,linkatom=self.linkatoms, mix_style=self.mix_style, output_file=self.output_file)
        demo_model.main2()
