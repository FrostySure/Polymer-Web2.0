...
class Workflow_crystal():
    def __init__(self, output_file, pc_list, npoly,  linkatoms, box_size_list, nchain,  mix_style='n',npt_step=1000, nvt_step=5000, ncore='6'):
    ...
    def step_1(self):
        ...
        demo_model = write_lts.Car2lt(pc_list=self.pc_list, npoly=self.npoly, tes_chain=self.nchain[0], box_size_list=self.box_size_list,
                                        lbox_size_list=self.box_size_list,linkatom=self.linkatoms, mix_style=self.mix_style, output_file=self.output_file)
        demo_model.main()

        demo_model = write_order_lts.Car2lt(pc_list=self.pc_list, npoly=self.npoly, tes_chain=self.nchain[0],box_size_list=self.box_size_list,
                                        lbox_size_list=self.box_size_list,linkatom=self.linkatoms, mix_style=self.mix_style, output_file=self.output_file)
        demo_model.main()
